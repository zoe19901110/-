      ...
      if (user) {
        const unsubscribeEnterprise = onSnapshot(doc(db, 'enterprises', currentIdentityId), (doc) => {
          if (doc.exists()) {
            ...
            const staticEnt = enterprises.find(e => e.id === currentIdentityId);
            setActiveEnterprise(staticEnt || null);
          }
        });
        return () => unsubscribeEnterprise();
      } else {
        // No user, use static data
        const staticEnt = enterprises.find(e => e.id === currentIdentityId);
        setActiveEnterprise(staticEnt || null);
      }
    } else {
  ...
  const switchIdentity = async (id: string) => {
    ...
    if (user) {
      try {
        ...
        console.warn('Failed to sync identity to Firestore:', e);
      }
    }
  };
